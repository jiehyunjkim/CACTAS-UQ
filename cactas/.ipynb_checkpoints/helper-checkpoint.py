...
class Helper:
    ...
    def predict_uncertainty(model, X, n_iter=10):
        all_means = []
        all_vars = []
        
        for i in range(X.shape[0]):
            slice_preds = np.array([
                model(np.expand_dims(X[i], axis=0), training=True).numpy()[0]
                for _ in range(n_iter)
            ])
            all_means.append(slice_preds.mean(axis=0))
            all_vars.append(slice_preds.var(axis=0))
    ...
    def ece(probs, labels, n_bins=10):
        ...
        for i in range(n_bins):
            ...
            accuracy = np.mean((probs[mask] > 0.5) == labels[mask])
            bin_prob = np.sum(mask) / len(probs)
            
            ece += np.abs(avg_confidence - accuracy) * bin_prob
    
        return ece

    def visualize_uncertainty(X_val, y_val, y_pred, uncertainty_map, start_index=100, end_index=110):
        ...
        for i in range(n):
            ...
            #plt.imshow(uncertainty_map[idx], cmap='hot')
            plt.imshow(uncertainty_map[idx], cmap='plasma', vmin=0, vmax=np.percentile(uncertainty_map[idx], 99))
    ...
    def evaluate_prediction_mean(prediction_mean, y_val, threshold=0.5):
        ious = []
        ious_thresh = []
    
        for i in range(len(prediction_mean)):
            pred_flat = prediction_mean[i].flatten()
            gt_flat = y_val[i].flatten()
    
            try:
                iou = jaccard_score(gt_flat.astype(int), (pred_flat > 0.0).astype(int))
                ious.append(iou)
            except:
                ious.append(0.0)
    
            try:
                iou_thresh = jaccard_score(gt_flat.astype(int), (pred_flat > threshold).astype(int))
                ious_thresh.append(iou_thresh)
            except:
                ious_thresh.append(0.0)
    
        return np.mean(ious), np.mean(ious_thresh) 
